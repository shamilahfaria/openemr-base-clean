...
connects to) is a real Chrome session against the running app — and Codex
...
what a user actually sees. This lets Codex investigate live behaviour
...
`takeScreenshot()` output that Codex can `Read` directly to render
the PNG inline.
